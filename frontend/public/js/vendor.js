/** 
 desabilita a variavel define e armazena na variavel 
 r para devolver apos o carregamento dos scripts
*/
...
var Helper = {
  ...
  /**
   Verifica se usuario esta navegando atraves de 
   dispositivo movel
   @method isMobile
   @todo: Aplicar logica no objeto Helper
  */
  isMobile: {
    Android: function() {
    ...
    any: function() {
      return (Helper.isMobile.Android() || 
              Helper.isMobile.BlackBerry() || 
              Helper.isMobile.iOS() || 
              Helper.isMobile.Windows());
    }
  },
  getRedirUrl: function(prefix, _section) {
    var c_url = '';
    if (this.isValidUrl(_section)) {
      if (!!PersistData[prefix + _section]) {
        console.log('getRedirUrl :: existe variavel');        
        return PersistData[prefix + _section];
      } else if(_section == 'home') {
          if (prefix == 'mbUrl'){
            return PersistData.mbUrl;
          } else{
            return PersistData.deskUrl;
          }
      } else {
        console.log('getRedirUrl :: nao existe variavel');
        return PersistData[prefix];
      }
    } else {
      console.log('getRedirUrl :: url nao e valida :: ' + _section);
    }
  },
  isValidUrl: function(_section) {
    if (_section === 'home') {
      return true;
    } else if(window.location.href.indexOf(_section) === -1){
      return false;
    } else {
        return true;
    }
  ...
  checkScreenWidth: function() {
    var _width = this.isLand() ? window.screen.height : window.screen.width;
    console.log(this.isLand());
    console.log(_width);
    return _width;
  }, 
  isLand: function() {
    var orientation = (typeof window.orientation === "undefined") ? null : window.orientation;
    var isLand = (orientation) ? ((orientation == 90 || orientation == -90) ? true : false) : false;
    return isLand;
  }
};
